...
$(document).ready(function(){
	...
	var biquadNode = audioCtx.createBiquadFilter();
	...
	biquadNode.frequency.value = 3000;
	biquadNode.Q.value = 1;
	biquadNode.gain.value = 1;
	...
	$("#sliderFrequency").change(function(){
		biquadNode.frequency.value = parseInt($(this).val());
		$("#frequencyLabel").text(parseInt($(this).val())+" Hz");
	});
	$("#gain").change(function(){
		biquadNode.gain.value = parseInt($(this).val());
		$("#labelGain").text(parseInt($(this).val()));
	});

	$("#biquadQ").change(function(){
		biquadNode.Q.value = parseFloat($(this).val());
		$("#labelFatorQ").text(parseFloat($(this).val()));
	});
	...
	$("#selectFilter").change(function(){
		...
		switch(selected){
		    ...
		    case "selectParam":
		    	disconnectAll();
		        activeDSP=biquadNode;
		        reconectNodes();
		        $("#paramFilter").show(500);
		        $("#customFilter").hide(500);
				$("#firCoef").hide(500);
				timeOut='?';	
		        break;
	...
	$("#selectBiquadType").change(function(){
		biquadNode.type = $(this).val();
	});
	...
	$("#customGET").click(function(){
		var customInit = $("#customEditorInit").val();
		var customCode = $("#customEditor").val();
		var customFunction=customInit+";scriptCustomNode.onaudioprocess = function(audioProcessingEvent) {var dIn = new Date();var timeIn = dIn.getTime();"+customCode+"var dOut = new Date();timeOut=dOut.getTime() - timeIn;}";
		eval(customFunction);
	});
